...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const changePassword = async (currentPassword: string, newPassword: string) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/users/change-password`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  const resetPassword = async (email: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/auth/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const resetPasswordConfirm = async (accessToken: string, newPassword: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/auth/update-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
